        ...
        for env_id, env_data in (fast.data or {}).items():
            ...
            for container in env_data.get("containers") or []:
                ...
                if cid not in known_container_ids:
                    ...
                    new.append(
                        DockhandContainerRestartButton(fast, client, env_id, env_name, base_url, container)
                    )
            ...
            for stack in env_data.get("stacks") or []:
                ...
                if sid not in known_stack_ids:
                    ...
                    new.append(
                        DockhandStackRestartButton(fast, client, env_id, env_name, base_url, stack)
                    )
...
class _BaseFastContainerButton(CoordinatorEntity[DockhandFastCoordinator], ButtonEntity):
...
class DockhandStackRestartButton(_BaseFastStackButton):
    ...
    def __init__(self, coordinator: DockhandFastCoordinator, client: Any,
        ...
        self._attr_unique_id = f"dockhand_stack_{self._env_id}_{self._stack_name}_restart"
